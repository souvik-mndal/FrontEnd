...
function Add({ title, price , thumbnail }) {
...
function App() {
  return (
    <>
      <h1>Data Show ( little dynamic )</h1>
      <div className="bigWrap">
        {carts.map((data) => {
          let val = data.products;
          return val.map((inner) => {
              return (<div className="wrap" key={inner.id}>
                <Add title={inner.title} price={inner.price} thumbnail={inner.thumbnail}/>
              </div>)
          });
        })}
      </div>
